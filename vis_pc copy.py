...
def process_numpy_to_pytorch(pointclouds_np):
    # Convert numpy array to torch tensor
    from pytorch3d.structures import Pointclouds
    pointclouds_torch = torch.tensor(pointclouds_np, dtype=torch.float32)
    
    # Ensure the tensor is of shape [N, 6]
    if pointclouds_torch.shape[1] != 6:
        raise ValueError("Input point clouds must have shape [N, 6]")

    xyz = pointclouds_torch[:, :3]
    center=xyz-xyz.mean(0)
    max_d=torch.norm(xyz, p=2, dim=1).max()
    xyz_norm = xyz / max_d
    rgb = pointclouds_torch[:, 3:]
    pointclouds_processed = Pointclouds(points=[rgb], features=[xyz])

    return pointclouds_processed
...
def visualize(seq, exp):
    ...
    RENDER_MODE='color'
    w2c, k = (np.array((json_file['w2c'])[0][0]), np.array(json_file['k'][0][0]))
    init_pt_cld = np.load(f"./data_ego/{seq}/init_pt_cld.npz")["data"]
    pointclouds = process_numpy_to_pytorch(init_pt_cld[:, :6])





# Assuming `xyz_normalized` is your normalized 3D point cloud (as a PyTorch tensor)
# and `device` is your computation device (CPU or GPU)

# Convert your `w2c` and `k` matrices to PyTorch tensors if they aren't already
w2c_tensor = torch.tensor(w2c, dtype=torch.float32, device=device)
k_tensor = torch.tensor(k, dtype=torch.float32, device=device)

# Invert `w2c` since PyTorch3D uses camera-to-world transformation matrices
c2w_tensor = torch.inverse(w2c_tensor)

# Extract intrinsic parameters
focal_length = (k_tensor[0, 0], k_tensor[1, 1])  # Assuming fx and fy are on the diagonal of K
principal_point = (k_tensor[0, 2], k_tensor[1, 2])  # Assuming cx and cy are the last elements of the first two rows of K

# Create a camera in PyTorch3D
camera = PerspectiveCameras(
    focal_length=focal_length,
    principal_point=principal_point,
    R=c2w_tensor[:3, :3].unsqueeze(0),  # Rotation part of the camera-to-world matrix
    T=c2w_tensor[:3, 3].unsqueeze(0),  # Translation part as a 3x1 vector
    device=device
)

# Project the 3D points to 2D using the camera
# Transform the points from world space to camera space
points_camera_space = camera.transform_points(xyz_normalized.unsqueeze(0))

# Apply perspective projection to get 2D points on the image plane
# This involves dividing by the z-coordinate
points_image_plane = points_camera_space[..., :2] / points_camera_space[..., 2:3]

# The points on the image plane might need scaling and translation depending on your specific setup
# This could involve multiplying by the focal length and adding the principal point, for example

# Note: This is a simplified projection model. Depending on your specific requirements, you might need to
# account for distortion coefficients or other intrinsic parameters.
points_image_plane = torch.rand(100, 2) * 2 - 1  # Generating random points in range [-1, 1] for both x and y

# Convert points to image coordinates (assuming image size of 1024x1024 for demonstration)
image_size = 1024
points_image_coordinates = ((points_image_plane + 1) / 2.0) * image_size  # Scaling and translating points

# Plotting
plt.figure(figsize=(10, 10))
plt.scatter(points_image_coordinates[:, 0], image_size - points_image_coordinates[:, 1], s=1)  # Flipping y-axis for image coordinates
plt.xlim(0, image_size)
plt.ylim(0, image_size)
plt.axis('off')  # Hide axes

# Save the image
image_path = '/mnt/data/pointcloud_projection.png'
...
'''
    ...
    while len(frames)<50:
        ...
        num_loops = 1.4
        y_angle = extract_y_angle(w2c)+360*t*num_loops / num_timesteps
        frame=render_pointcloud_pytorch3d(w, h, image_size, radius, pointclouds, y_angle)

        array = (frame * 255).astype(np.uint8)  # Scale to 0-255 and convert to uint8
        frame = Image.fromarray(array)
        frames.append(frame)
        frame.save(os.path.join('/data3/zihanwa3/Capstone-DSR/Dynamic3DGaussians/vis/frames', f'frame_{len(frames)}.png'))

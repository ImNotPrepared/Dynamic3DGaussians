...
def get_dataset(t, md, seq, mode='stat_only'):
    ...
    if mode=='ego_only':
      for c in range(1,5):
          ...
          fn = fn.split('/')[-1]
          paath = f"/data3/zihanwa3/Capstone-DSR/Processing_dance/undist_data/undist_cam0{c}/{fn}"
          print(paath)
...
def initialize_params(seq, md):
    dust_pc_path = '/data3/zihanwa3/Capstone-DSR/Appendix/dust3r/duster_depth_clean_dance_512/1543/pc.npz'
...
def train(seq, exp):
    ...
    reversed_range = list(range(0, 500, 3))
    for t in reversed_range:
        ...
        num_iter_per_timestep = int(2.3e4) if is_initial_timestep else int(2.1e3)

...
def get_dataset(t, md, seq):
    ...
    for c in range(len(md['fn'][t])):
        ...
        fn = md['fn'][t][c]
        im = np.array(copy.deepcopy(Image.open(f"./data_ego/{seq}/ims/{fn}")))
        im = torch.tensor(im).float().cuda().permute(2, 0, 1) / 255
        seg = np.array(copy.deepcopy(cv2.imread(f"./data_ego/{seq}/seg/{fn.replace('.jpg', '.png')}", cv2.IMREAD_GRAYSCALE))).astype(np.float32)
...
def train(seq, exp):
    ...
    for t in range(10):
        ...
        num_iter_per_timestep = 30000 if is_initial_timestep else 5000

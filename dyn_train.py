...
def get_dataset(t, md, seq, mode='stat_only'):
    ...
    if mode=='ego_only':
      for c in range(5):
          ...
          if h == w:
            mask_path=f'/ssd0/zihanwa3/data_ego/cmu_bike/depth/{int(c)}/depth_{t}.npz'
            ...
            depth = torch.tensor(np.load(mask_path)['depth_map']).float().cuda()
            mask_path=f'/ssd0/zihanwa3/data_ego/cmu_bike/depth/{int(c)}/depth_0.npz'
...
def initialize_optimizer(params, variables):
    lrs = {
        'means3D': 0.000014 * variables['scene_radius'],
        'rgb_colors': 0.0000028,
        'seg_colors': 0.0,
...
def train(seq, exp):
    ...
    for t in range(111):
        ...
        num_iter_per_timestep = int(7.7) if is_initial_timestep else 2

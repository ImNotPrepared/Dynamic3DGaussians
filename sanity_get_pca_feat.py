...
with open('/data3/zihanwa3/Capstone-DSR/Processing/dinov2features/fitted_pca_model.pkl', 'wb') as f:
    pickle.dump(pca, f)
print('dumped!!!!')
# Step 3: Visualize features using PCA
#3 for ts in tqdm(range(123, 423), desc=f"Visualizing features for cam {cam_id}"):
for cam_id in range(1, 5):
  for ts in tqdm(range(183, 294), desc=f"Visualizing features for cam {cam_id}"):
      ...
      # Save the image
      base_base_path = f'/data3/zihanwa3/Capstone-DSR/Processing/dinov2features/pca_aligned_Oct17/{cam_id}'

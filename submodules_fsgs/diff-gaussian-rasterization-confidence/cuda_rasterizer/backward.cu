...
__global__ void computeCov2DCUDA(int P,
	...
	const int* radii,
	const float* cov3Ds,
  const int width, int height, //added
	...
	float* dL_dcov)
{
  ...
  // added 
  const float lim_x_pos = (width - c_x) / h_x + 0.3f * tan_fovx;
  const float lim_x_neg = c_x / h_x + 0.3f * tan_fovx;
  const float lim_y_pos = (height - c_y) / h_y + 0.3f * tan_fovy;
  const float lim_y_neg = c_y / h_y + 0.3f * tan_fovy;

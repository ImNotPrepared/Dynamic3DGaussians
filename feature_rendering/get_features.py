...
def main(args):
    ...
    iiiiiiiiiiiids = list(range(49, 350, 3))
    for j, seq in enumerate(seqs):
        ...
        path = f'{args.input_dir}/{args.seq.split("_")[2]}_{seq}/*.jpg'
        paths = glob.glob(path)
        sorted_paths = sorted(paths, key=lambda x: int(os.path.basename(x).split('.')[0]))
    ...
    with open(f'/data3/zihanwa3/Capstone-DSR/Processing{args.seq}/dinov2features/fitted_pca_model_fg_only_is_{fg_only}.pkl', 'wb') as f:
        pickle.dump(pca, f)
    print('dumped!!!!')
...
if __name__ == "__main__":
    ...
    args.save_dir = f'/data3/zihanwa3/Capstone-DSR/Processing{args.seq}/dinov2features/resized_512_Aligned_fg_only_is_{str(args.fg_only)}'
